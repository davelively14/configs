...
The only real change between this and [release candidate 0](https://github.com/davelively14/configs/blob/master/phoenix_1.3.0-rc.0_react_redux.md)), is in the webpack configuration. In Phoenix 1.3.0, the `web` directory was moved from `lib/app_name/web` to `lib/app_name_web`.
...
* Within the `assets` directory, create a file named `.babelrc` in order to config babel and add the following:
```json
{
...
* Create the `webpack.config.js` file in the `/assets` directory and configure like this (note: this is for Webpack 2 - it won't work if you're using an older version of Webpack). **Also, replace [ADD ADD_APP_NAME_IN_SNAKE_CASE] with your own app.**
```javascript
'use strict'
...
function web(dest) { return root('lib/[ADD_APP_NAME_IN_SNAKE_CASE]_web/static/' + dest) }
